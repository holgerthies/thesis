...
\section{iRRAM}
  ...
  the user.
   ...
   \irram further provides classes for complex numbers and real matrices.
      ...
      \item \textbf{compute function}
        The \irram framework already writes the \cc \code{main} function, so
  ...
  boolean}
    ...
    When an expression evaluates to $\bot$, it means that the current information is not sufficient to make a decision.
		When casting to \code{bool} $\bot$ leads to reiteration. 
    ...
    gcc} compiler even has a warning option on that:
		\begin{quotation}
				\noindent
		\begin{verbatim}
			-Wfloat-equal
